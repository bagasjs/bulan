...
bool compile_program(Lexer *lex)
{
    ...
    while(lexer_get_token(lex) && lex->token != TOKEN_EOF) {
        ...
        Function fn = {0};
        fn.arena = arena;
        fn.name = arena_strdup(&arena, lex->string);
        push_block(&fn);
        ...
        if(!codegen_fasm_x86_64_win32(&output, &fn)) {
            compiler_diagf(fn_loc, "Failed to compile function %s", fn.name);
        }
